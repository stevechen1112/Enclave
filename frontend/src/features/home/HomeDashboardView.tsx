...
function Stat({ label, value, tone = 'normal', to, hint }: { label: string; value: number; tone?: 'normal' | 'warning' | 'danger'; to?: string; hint?: string }) {
  ...
  const body = <><dt className="text-sm font-medium">{label}</dt><dd className="mt-1 text-3xl font-bold tabular-nums">{value}</dd>{hint && <dd className="mt-1 text-xs opacity-75">{hint}</dd>}</>
  return to ? <Link to={to} className={`rounded-2xl border p-4 transition hover:-translate-y-0.5 hover:shadow-sm ${colors}`}>{body}</Link> : <div className={`rounded-2xl border p-4 ${colors}`}>{body}</div>
}

export default function HomeDashboardView({ model }: { model: HomeDashboardModel }) {
  const todos = [
    model.canReview && model.stats.review > 0 ? { key: 'review', icon: Inbox, title: `${model.stats.review} 個來源等待人工確認`, to: '/knowledge/review', action: '開始確認' } : null,
  ...
  return <div className="h-full overflow-y-auto">
    ...
    <div className="mx-auto max-w-5xl space-y-7 px-5 py-7">
      <AsyncState loading={model.loading} error={model.error} onRetry={model.reload}>
        ...
        <section aria-labelledby="knowledge-health"><h2 id="knowledge-health" className="text-xl font-semibold text-ink">知識健康與處理狀態</h2><p className="mt-1 text-sm text-muted">每個數字都以「來源檔案」計算；點選即可查看下一步。</p><dl className="mt-3 grid grid-cols-2 gap-3 lg:grid-cols-5"><Stat label="全部來源" value={model.stats.total} to="/knowledge/assets" /><Stat label="已可問答" value={model.stats.ready} to="/knowledge/assets?status=answer_ready" hint="可到問知識使用" /><Stat label="系統處理中" value={model.stats.processing} to="/knowledge/assets?status=processing" /><Stat label="等待人工確認" value={model.stats.review} tone={model.stats.review ? 'warning' : 'normal'} to="/knowledge/review" hint={model.stats.reviewItems ? `內含 ${model.stats.reviewItems} 筆候選內容` : undefined} /><Stat label="需要處理" value={model.stats.failed} tone={model.stats.failed ? 'danger' : 'normal'} to="/knowledge/assets?status=needs_attention" /></dl></section>
