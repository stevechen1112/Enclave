...
def _interview_handler(ctx: TaskRunContext) -> TaskResult:
    ...
    card = repo.create_knowhow(
        tenant_id=ctx.user.tenant_id,
        title=ctx.inputs.get("title") or "未命名訪談",
        summary=ctx.inputs.get("summary") or "",
        steps=ctx.inputs.get("steps") or [],
        data={"source": "interview", "task_run_id": str(ctx.run.id)},
        owner_id=ctx.user.id,
    )
    return TaskResult(
        output_refs={"knowhow_card_id": str(card.id)},
        provenance={"handler": "interview", "knowhow_card_id": str(card.id)},
        next_status="waiting_review",
    )
