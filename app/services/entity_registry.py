...
def resolve_entity(db, *, tenant_id, value: str, entity_type: str | None = None) -> EntityResolution:
    normalized = normalize_entity(value)
    query = db.query(EntityRegistry).filter(EntityRegistry.tenant_id == tenant_id, EntityRegistry.status == "active")
    if entity_type:
        query = query.filter(EntityRegistry.entity_type == entity_type)
    direct = [entity for entity in query.all() if normalize_entity(entity.canonical_key) == normalized or normalize_entity(entity.display_name) == normalized]
    aliases = db.query(EntityAlias).join(EntityRegistry, EntityAlias.entity_id == EntityRegistry.id).filter(
...
def add_alias(db, *, entity: EntityRegistry, alias: str, approved: bool, source_ref: dict | None = None) -> EntityAlias:
    normalized = normalize_entity(alias)
    if not normalized:
        raise ValueError("entity alias cannot be empty")
    row = EntityAlias(tenant_id=entity.tenant_id, entity_id=entity.id, alias=alias.strip(),
                      alias_normalized=normalized, approved=approved, source_ref=source_ref or {})
