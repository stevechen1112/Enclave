...
def list_documents(
    ...
    if department_id and not can_access_document_by_department(
    ...
    can_manage = current_user.is_superuser or current_user.role in {
        "owner",
        "admin",
        "hr",
    }
    ...
    documents = (
        query.order_by(DocumentModel.created_at.desc()).offset(skip).limit(limit).all()
    )
...
async def upload_document(
    *,
    ...
    # 5. 建立文件記錄
    doc_in = DocumentCreate(filename=clean_filename, file_type=file_type)
    ...
    try:
        ...
        try:
            crud_document.tombstone(
                db, document_id=document.id, reason="storage_put_failed"
            )
        except Exception:
            logger.error(
                "tombstone after storage put failure also failed: doc=%s", document.id
            )
        logger.warning("object storage unavailable during upload: doc=%s", document.id)
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="檔案儲存服務暫時不可用；資料未發布，請稍後重試",
        ) from exc
...
def delete_document(
    *,
    ...
    if not current_user.is_superuser and document.tenant_id != current_user.tenant_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="無權限刪除此文件"
        )
