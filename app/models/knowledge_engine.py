...
class KnowledgeBaseRevisionDocument(Base):
    ...
    kb_revision_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_base_revisions.id"), nullable=False, index=True)
    document_id = Column(UUID(as_uuid=True), ForeignKey("documents.id"), nullable=False, index=True)
    document_version_id = Column(UUID(as_uuid=True), ForeignKey("documentversions.id"), nullable=False, index=True)
    ...
    __table_args__ = (
        UniqueConstraint("kb_revision_id", "document_id", name="uq_kb_revision_document"),
        UniqueConstraint("kb_revision_id", "document_version_id", name="uq_kb_revision_document_version"),
    )
...
class PolicySnapshot(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    __table_args__ = (UniqueConstraint("kb_id", "revision", name="uq_kb_policy_snapshot"),)
...
class IndexArtifactRevision(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    __table_args__ = (UniqueConstraint("kb_revision_id", "artifact_type", "namespace", name="uq_kb_index_artifact"),)
...
class DocumentProfile(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    __table_args__ = (
        UniqueConstraint("document_id", "document_revision", name="uq_document_profile_revision"),
...
class StructuredTable(Base):
    ...
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    __table_args__ = (UniqueConstraint("document_id", "document_revision", "table_key", name="uq_structured_table_key"),)
...
class StructuredRow(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    table_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_structured_tables.id"), nullable=False, index=True)
    row_key = Column(String(255), nullable=False)
    row_number = Column(Integer, nullable=False)
    identity_json = Column(JSON, nullable=False, default=dict)
    row_hash = Column(String(64), nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    fields = relationship("StructuredField", cascade="all, delete-orphan", lazy="selectin")
    __table_args__ = (UniqueConstraint("table_id", "row_key", name="uq_structured_row_key"),)
...
class StructuredField(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    row_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_structured_rows.id"), nullable=False, index=True)
    ...
    bbox = Column(JSON, nullable=True)
    __table_args__ = (UniqueConstraint("row_id", "field_name", name="uq_structured_row_field"),)
...
class ProcedureGraph(Base):
    ...
    content_hash = Column(String(64), nullable=False)
    phases = relationship("ProcedurePhase", cascade="all, delete-orphan", lazy="selectin")
    __table_args__ = (UniqueConstraint("document_id", "document_revision", "title", name="uq_procedure_graph"),)
...
class ProcedurePhase(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    graph_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_procedure_graphs.id"), nullable=False, index=True)
    ...
    __table_args__ = (UniqueConstraint("graph_id", "phase_key", name="uq_procedure_phase"),)
...
class EntityAlias(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    entity_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_entities.id"), nullable=False, index=True)
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "alias_normalized", "entity_id", name="uq_tenant_entity_alias"),)
...
class KnowledgeRelease(Base):
    ...
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    kb_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_bases.id"), nullable=False, index=True)
    kb_revision_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_base_revisions.id"), nullable=False)
    runtime_release_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_runtime_releases.id"), nullable=True)
    status = Column(String(24), nullable=False, default="candidate")
...
class RollbackPoint(Base):
    ...
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    kb_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_bases.id"), nullable=False, index=True)
    from_release_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_releases.id"), nullable=False)
    to_release_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_releases.id"), nullable=False)
...
class EvaluationRun(Base):
    ...
    baseline_run_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_evaluation_runs.id"), nullable=True)
    summary_json = Column(JSON, nullable=False, default=dict)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    completed_at = Column(DateTime(timezone=True), nullable=True)
    __table_args__ = (
        Index("uq_knowledge_eval_first_run", "evaluation_key", unique=True,
...
class EvaluationCaseResult(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    run_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_evaluation_runs.id"), nullable=False, index=True)
...
class EvaluationHumanReview(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    case_result_id = Column(UUID(as_uuid=True), ForeignKey("knowledge_evaluation_case_results.id"), nullable=False, index=True)
...
class KnowledgeFreshnessState(Base):
    ...
    __table_args__ = (UniqueConstraint("document_id", name="uq_document_freshness_state"),)
...
class LexicalIndexEntry(Base):
    ...
    chunk_id = Column(UUID(as_uuid=True), ForeignKey("documentchunks.id"), primary_key=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    document_id = Column(UUID(as_uuid=True), ForeignKey("documents.id"), nullable=False, index=True)
    document_revision = Column(Integer, nullable=False)
    ...
    __table_args__ = (Index("ix_knowledge_lexical_tokens_gin", "tokens", postgresql_using="gin"),)
