...
class KnowhowCardModel(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    owner_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)  # 建立者；PATCH/submit 授權依據
...
class MKATaskCost(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    task_type = Column(String, nullable=False, index=True)  # stt | tts | form_export | chat | ...
...
class SceneRegistry(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "token", name="uq_mka_scene_registry_tenant_token"),
    )
...
class UserJobRoleAssignment(Base):
    ...
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=False, index=True)
    job_role_id = Column(UUID(as_uuid=True), ForeignKey("mka_job_roles.id"), nullable=False, index=True)
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "user_id", "job_role_id", name="uq_mka_user_job_role"),
    )
...
class TaskDefinition(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=True, index=True)  # null = 全域定義
    task_key = Column(String, nullable=False, index=True)  # ask | quote | incident | handover | quality_8d | interview | training | daily_report
    name = Column(String, nullable=False)
    description = Column(Text, nullable=True)
    version = Column(String, default="1.0", nullable=False)
    status = Column(String, default="draft", nullable=False)  # draft | enabled | disabled | deprecated
    ...
    approval_policy_id = Column(UUID(as_uuid=True), nullable=True)
    output_bindings = Column(JSON, default=list)  # [{"kind": "form", "form_key": "quote"}, ...]
    risk_level = Column(String, default="low", nullable=False)  # low | medium | high
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(DateTime(timezone=True), onupdate=func.now())

    __table_args__ = (
        UniqueConstraint("tenant_id", "task_key", "version", name="uq_mka_task_def_key_version"),
    )
...
class TaskRun(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    task_definition_id = Column(UUID(as_uuid=True), ForeignKey("mka_task_definitions.id"), nullable=False)
    ...
    provenance = Column(JSON, default=dict)  # 文件/版本/規則/工具/模型/confidence/missing/manual_edits
...
class TaskRunEvent(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    run_id = Column(UUID(as_uuid=True), ForeignKey("mka_task_runs.id"), nullable=False, index=True)
...
class FormTemplate(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "form_key", "version", name="uq_mka_form_template_version"),
    )
...
class MKAWriteRequest(Base):
    ...
    __table_args__ = (
        UniqueConstraint("tenant_id", "idempotency_key", name="uq_mka_write_idempotency"),
...
class MKAWriteAudit(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    write_request_id = Column(UUID(as_uuid=True), ForeignKey("mka_write_requests.id"), nullable=True)
    correlation_id = Column(String, nullable=False, index=True)
...
class KnowhowLineage(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, index=True)
    tenant_id = Column(UUID(as_uuid=True), ForeignKey("tenants.id"), nullable=False, index=True)
    card_id = Column(UUID(as_uuid=True), ForeignKey("knowhow_cards.id"), nullable=False, index=True)
