...
| LLM 客戶端 | `app/services/llm_client.py` | OpenAI / Ollama 統一介面，可熱切換 |
...
| 變數 | 預設值 | 說明 |
|---|---|---|
| `LLM_PROVIDER` | `openai` | `openai`、`gemini` 或 `ollama` |
| `OPENAI_API_KEY` | *(必填)* | OpenAI API 金鑰 |
| `OPENAI_MODEL` | `gpt-4o-mini` | 問答 / 生成模型 |
...
| `VOYAGE_API_KEY` | *(必填)* | Voyage AI 嵌入金鑰 |
| `VOYAGE_MODEL` | `voyage-4-lite` | 嵌入模型 |
...
| POST | `/agent/scan-preview` | Wizard：Ollama AI 分析子資料夾並生成摘要 | admin |
