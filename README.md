...
│  ├── 14 個 API 路由模組（analytics / sso 暫時停用）                                          │
...
│    我的報告         │   /reports
│    文件管理         │   /documents
│    用量統計         │   /usage              (全員；admin 4 tabs，member 1 tab)
├─────────────────────┤
│  管理               │ ← owner / admin / manager
│    Agent 設定       │   /agent              (資料夾設定 + 處理進度 tab)
│    審核佇列         │   /agent/review
├─────────────────────┤
│  分析               │ ← owner / admin
...
| `/agent` | AgentPage | admin | Agent 資料夾設定（處理進度已合入 `?tab=progress` tab） |
| `/agent/review` | ReviewQueuePage | admin | 待審核清單 |
| `/query-analytics` | QueryAnalyticsPage | admin | 問答分析（概覽/熱門問題/知識缺口） |
| `/usage` | UsagePage | viewer | 用量統計（admin：4 tabs 總覽/部門/成員/個人；member：僅個人 tab） |
...
> **合併重定向**：`/rag-dashboard` → `/query-analytics`、`/usage-report` → `/usage`、`/my-usage` → `/usage?tab=personal`、`/agent/progress` → `/agent?tab=progress`
...
| `FIRST_SUPERUSER_PASSWORD` | *(必填，無預設值)* | 初始 superuser 密碼，**必須在 .env 中設定** |
...
│   │   └── v1/
│   │       ├── api.py          # 路由總覽（16 個端點檔案；analytics / sso 路由停用，14 個啟用）
│   │       └── endpoints/      # 17 個端點模組（1 個暫停路由）
│   │           ├── auth.py
│   │           ├── users.py
...
│   │           ├── kb.py
│   │           ├── chat.py          # 問答 + 串流 + 多輪對話
│   │           ├── chat_analytics.py# 問答日誌分析（P10-5；admin+）
│   │           ├── generate.py      # 內容生成 + 匯出
│   │           ├── reports.py       # 報告管理 CRUD
│   │           ├── agent.py         # Agent + 審核佇列
│   │           ├── kb_maintenance.py
│   │           ├── audit.py
│   │           ├── analytics.py     # 成本分析（T3-5；費用統計）
│   │           ├── departments.py
│   │           ├── feature_flags.py
│   │           ├── sso.py           # ← 路由暫時停用（功能未完成）
│   │           └── mobile.py
...
│   │   ├── analytics.py        # QuerySummary, DailyQueryCount, TopQuery, UnansweredQuery
│   │   ├── admin.py            # OrgDashboard, AdminUserInfo, SystemHealth
│   │   ├── chat.py             # ChatRequest/Response, Feedback（含 Literal 型別）
│   │   ├── user.py             # UserRoleLiteral, UserStatusLiteral
│   │   └── token.py            # TokenPayload（含 tenant_id）
│   ├── crud/                   # 資料存取層
│   ├── services/               # 業務邏輯
...
1. 從 [app/config.py](app/config.py) 掌握所有可調變數（含預設值）
2. 閱讀 [app/api/v1/api.py](app/api/v1/api.py) 了解路由總覽（哪個 prefix 對應哪個模組）
3. 閱讀 [app/api/deps.py](app/api/deps.py) 了解依賴注入（`get_db`, `get_current_active_user`）
4. 查看 [app/services/kb_retrieval.py](app/services/kb_retrieval.py) 了解 RAG 核心邏輯
5. 執行 `docker compose up -d && alembic upgrade head` 啟動開發環境

### 核心設計慣例

| 慣例 | 說明 |
|---|---|
| **租戶隔離** | 所有 DB query 必須套用 `.filter(Model.tenant_id == current_user.tenant_id)`；缺此條件則為安全漏洞 |
| **依賴注入** | `db: Session = Depends(deps.get_db)` 取得 Session；切勿在 endpoint 裡直接 `SessionLocal()` |
| **RBAC 檢查** | 使用 `app/api/deps_permissions.py` 中的 `require_*` 函式；若需自訂則比對 `current_user.role` |
| **Celery 任務** | 向量化等耗時操作走 `app/tasks/document_tasks.py`；endpoint 只做排隊，不等結果 |
| **SSE 串流** | 問答與生成端點輸出 `text/event-stream`；格式固定為 `data: <JSON>\n\n`，前端用 `EventSource` 接收 |
| **Pydantic Schema 位置** | 所有 request/response Schema 集中放在 `app/schemas/`；endpoint 檔案內**不應**定義 inline class |
| **型別標注** | 使用 `Literal["admin", "owner", ...]` 限縮字串枚舉；`UserRoleLiteral` 等別名定義在 `schemas/user.py` |
| **日誌** | 使用 `logging.getLogger(__name__)`；message 用 `%s` lazy 格式，不要用 f-string |

### 常見陷阱

- **JSON 欄位預設值**：`Column(JSON, default=dict)`，**不是** `default={}`（後者會被多個實例共享）
- **Celery 任務 N+1**：批次處理時先用 ID set pre-fetch，見 `document_tasks.py` 的預取模式
- **Rate Limit Redis**：使用 `REDIS_HOST:REDIS_PORT db=2`，與 Celery broker（db=0）和檢索快取（db=1）隔離
- **superuser vs owner**：`is_superuser=True` 可跨租戶操作；`role=owner` 僅限本租戶最高權限
- **`structured_answers.py`**：`EmployeeRoster.load()` 和 `try_structured_answer()` 接受可選的 `db` 參數；有上游 Session 時請傳入，避免重複建立短命連線

### 新增端點的標準流程

```
1. app/schemas/<feature>.py    — 定義 request/response Pydantic models
2. app/crud/crud_<feature>.py  — 定義 DB 查詢邏輯（封裝 SQLAlchemy query）
3. app/api/v1/endpoints/<feature>.py  — 定義 FastAPI router
4. app/api/v1/api.py           — include_router(...)
5. alembic revision --autogenerate  — 若有新 Model
```
...
| [docs/reports/INTEGRATION_AUDIT_REPORT.md](docs/reports/INTEGRATION_AUDIT_REPORT.md) | 整合審查報告（Phase 1-13） |
| [docs/reports/INTEGRATION_FIX_REPORT.md](docs/reports/INTEGRATION_FIX_REPORT.md) | 整合修復完成報告 |
