...
@pytest.mark.asyncio
async def test_uploaded_document_appears_in_list(
...
@pytest.mark.asyncio
async def test_same_document_content_reuses_existing_source(
    client: AsyncClient, superuser_headers: dict
):
    """重新選取相同檔案不得建立第二個文件或知識來源。"""
    tenant = await create_tenant(client, superuser_headers, {"name": "DocumentDedupCo"})
    await create_user(
        client,
        superuser_headers,
        {
            "email": "hr@document-dedup.example",
            "password": "HR123!",
            "full_name": "HR",
            "role": "hr",
            "tenant_id": tenant["id"],
    ...
    headers = await login_user(client, "hr@document-dedup.example", "HR123!")

    first = await client.post(
        "/api/v1/documents/upload",
        headers=headers,
        data={"idempotency_key": "document-attempt-1"},
        files={"file": ("same.txt", TXT_CONTENT, "text/plain")},
    )
    second = await client.post(
        "/api/v1/documents/upload",
        headers=headers,
        data={"idempotency_key": "document-attempt-2"},
        files={"file": ("same.txt", TXT_CONTENT, "text/plain")},
    )
...
@pytest.mark.asyncio
async def test_cross_tenant_document_inaccessible(
    client: AsyncClient, superuser_headers: dict
):
    """跨租戶文件存取應被拒絕"""
